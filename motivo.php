...
$motivo= new motivo_v();
  ...
  {  $motivo= new motivo_v($_POST["ocu_N"], $_POST["nom"]);
  	
		if (isset($_POST["ocu_N"]) && $_POST["ocu_N"]!='0')
		{
			$gua=$motivo->modificar_motivo_v($_POST["ocu_N"]);
			if ($gua)
			{	
		...
		if (isset($_POST["ocu_N"]) && $_POST["ocu_N"]=='0')
		{
			...
			{
			$gua=$motivo->guardar_motivo_v($_SESSION["cedu_usu"]);
			if ($gua)
			{
	...
	{	//$gua=$dia->eliminar($_POST["ocu_N"]);
		$gua=$motivo->eliminar_motivo($_POST["ocu_N"]);
		if ($gua)
			{
...
?>
  <table width="619" border="0" align="center" bgcolor="#FFFFFF">
    <tr class="titulofor">
      <td colspan="3"><div align="center">Motivo de Visita</div></td>
    </tr>
    <tr>
      <td width="73" class="etiqueta">Motivo:</td>
      ...
      <span class="textoN">M&aacute;ximo 150 caracteres.</span></td></tr>
     <tr>
 ...
 $ver=$motivo->ver_motivo_v();
        if ($ver==false)
		{
		   echo "<script>alert('Error para mostrar los motivos de visita');</script>";
		} 
		else
		{
		    echo $ver;
