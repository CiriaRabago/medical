...
/* FUNCIÓN PARA LLENAR COMBO DE SERVICIOS */  		
	function combo_servicios()
	{
	   	$sql="SELECT 
			...
			nomb_servicio
			from slc_servicio
			where elimina_servicio='0' and status_servicio='A'
...
function lista_serv_tab_emp()
	{
	   	$sql="SELECT s.nomb_servicio, s.id_servicio, s.precio_servicio
		from slc_servicio s where s.status_servicio='A' and s.elimina_servicio='0' order by s.nomb_servicio ";
...
{	    
	...
	function listado_fact2($des,$has,$serv,$pac,$edo,$emp,$med)
	{
		...
		if($med!='') 
			...
			,s.nomb_servicio,m.ced_rif_medico,m.nomb_medico,a.slc_servicio_id_servicio,a.id_empresa
	...
	function listado_atenc_res($des,$has,$serv,$pac,$edo,$emp,$med)
	{
		...
		{ 
			...
			order by ".$n_med."nomb_servicio, status_visita";
