...
{
 if(document.getElementById("getf_in").value!=''){
    getf_in=document.getElementById("getf_in").value;
    getf_fi=document.getElementById("getf_fi").value;
    getser=document.getElementById("getser").value;
    getced=document.getElementById("getced").value;
    getemp=document.getElementById("getemp").value;
    getmed=document.getElementById("getmed").value;
 }
 
  window.location = "listado_fac_excel.php?getf_in="+getf_in+"&getf_fi="+getf_fi+"&getser="+getser+"&getced="+getced+"&getemp="+getemp+"&getmed"+getmed+"";
}
...
<?php if ($_POST["primera"]=='')
 ...
 ?>
<form name="form1" id="form1" method="post" action="listado_fac.php">
...
<input name="ocu_ff" id="ocu_ff" type="hidden" value="" />
	...
	if($_POST['ingreso']==1)
 	{
    	$reg=$ser->listado_fact2( $_POST['ocu_fi'],$_POST['ocu_ff'],$_POST['servicio'],$_POST['cedula'],$_POST['estado'],$_POST['empresa'],$_POST['medico']);//echo 'entro a verificar';
   		$cont=0;
      $precio=0;
		if($reg!=false)
		{
			$titulo='LISTADO DE ATENCION';
	...
	?> 
		  <table width="1040">
		  	<tr class="titulofor">
			  <td width="1040" colspan="15" align="center"><?php echo $titulo; ?></td>
        <input type="hidden" id="getf_in" value="<?phpecho $_POST[ocu_fi]?>">
        <input type="hidden" id="getf_fi" value="<?phpecho $_POST[ocu_ff]?>">
        <input type="hidden" id="getser" value="<?phpecho $_POST[servicio]?>">
        <input type="hidden" id="getced" value="<?phpecho $_POST[cedula]?>">
        <input type="hidden" id="getemp" value="<?phpecho $_POST[empresa]?>">
        <input type="hidden" id="getmed" value="<?phpecho $_POST[medico]?>">
...
<?php		   while ($row=mysql_fetch_array($reg))
       ...
       $busemple=$emple->buscar_nom_emple();
          ...
          <td width="150"><?php echo $row[6]; ?></td>
           <?php
           //consulta para ver beneficiario
	...
	<?php		}  ?>
        <tr class="titulorep" align="right">
          
          <td colspan="3"> TOTAL: <?phpecho $precio?></td>
        </tr>
				<tr >
				  <td colspan="14" align="center" class="td-buttons">
        ...
        <img src="imagenes/excel_icono" alt="Buscar" 
        width="50" height="50" title="Exportar a Excel" onclick="enviar_excel();"/> 
