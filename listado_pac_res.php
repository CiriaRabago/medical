...
function validar()
{
    ...
    check = document.getElementById("t_med");//valido si esta check la opcion detallada por medico
    if (check.checked) {
...
<?php  if ($_POST["primera"]=='')
 {
   ...
   $men= new menu('', '', '', 'listado_pac_res.php', '','', '', '');
...
<?php  /////////////////////////Mostrar u ocultar Botones///////////////////////////
 ...
 ?>
			...
			<tr class="titulofor">
			  <td height="30" colspan="5"><div align="center" class="titulofor">Listado Resumen de Pacientes</div></td>
			</tr>
...
<input name="ocu_ff" id="ocu_ff" type="hidden" value="" />
					...
					<?php   $ser= new servicio($_POST['servicio'],'','','','','','','','','','',''); ?> 
						<select name="servicio" class="texto" id="servicio" >
                          ...
                          <option value="" <?php  if($_POST['servicio']=="") echo ' selected ';?>>TODOS</option>
                  ...
                  <tr class="textoN">
                    <td height="21" colspan="3"><span class="textoN">MEDICO</span> </td>
                    </tr>				  
                  <tr>
                    <td colspan="3"><select name="medico" class="texto" id="medico">
					<option value="0" selected="selected" >SELECCIONE --></option>
                     ...
                     <td height="21" colspan="2"><span class="textoN">LISTADO DETALLADO DE MEDICOS<input name="t_med" id="t_med" type="checkbox"<?php  if ($_POST['ingreso']==2) {
        echo "checked='checked'"; } ?>/></span></td>
                  </tr>
                  <tr>
                    <td>&nbsp;</td>
                    <td>&nbsp;</td>
	...
	if($_POST['ingreso']==1)
 	{
    	$reg=$ser->listado_atenc_res( $_POST['ocu_fi'],$_POST['ocu_ff'],$_POST['servicio'],$_POST['cedula'],$_POST['estado'],$_POST['empresa'],$_POST['medico']);//echo 'entro a verificar';
   		$cont=0;
		if($reg!=false)
		{
...
<?php 	}
  ...
  {
      $reg=$ser->listado_atenc_res( $_POST['ocu_fi'],$_POST['ocu_ff'],$_POST['servicio'],$_POST['cedula'],$_POST['estado'],$_POST['empresa'],'X');//La X determina que se realiza la busqueda para todos los medicos;
      $cont=0;
    if($reg!=false)
    {
      $titulo='LISTADO RESUMEN DE ATENCIÓN';
  ...
  <?php     }  ?>
        <tr class="titulorep">
          <td colspan="4">TOTAL</td>
                <td width="10%"><?php  echo $acum; ?></td>
        </tr>
              <tr >
          <td colspan="12" align="center" class="td-btn">

          <a href="#" class="button-print" alt="Imprimir" <?php  echo $ope5;?>  onclick="irpdf('listado_pac_res_pdf.php?fi=<?php  echo $_POST['ocu_fi'];?>&ff=<?php  echo $_POST['ocu_ff'];?>&serv=<?php  echo $_POST['servicio'];?>&ced=<?php  echo $_POST['cedula'];?>&sta=<?php  echo $_POST['estado'];?>&empr=<?php  echo $_POST['empresa'];?>&medi=<?php  echo 'X'?>')"  > <i class="fa fa-print" aria-hidden="true"></i> Imprimir </a>
