...
if($_POST['guaocu']!='')
{
	...
	{ 
		...
		for($i=0;$i<$canticar;$i++)
		{
			...
			if($idcaract!="")
			{
				$guardet=$rr->mod_det_result($idcaract,$_POST[$idcar],$_POST[$idunime],$_POST[$idtabla]);
				if($guardet==false)
				{
				   $bandgua=false;
				}
...
else
{
	...
	if($inforesult==false)
		die( '<tr>
			<td  colspan="4" class="textoN">OCURRIO UN ERROR AL ACCEDER A LOS RESULTADOS</td>
		  </tr></table>');
	else
	  echo '<input name="idresulta" id="idresulta" type="hidden" value="'.$inforesult[2].'" />';
...
<?php							}
	...
	<?php		$bio= new empleado('','','','','','','','','','','','','','','','','','','');
		...
		if($result)
			...
			{
			   $selb='';
			   $bioantes=$inforesult[0];
			   if($row[0]==$inforesult[0])
			     $selb='selected';
			   echo '<option value="'.$row[0].'**'.$row[1].' '.$row[3].'**'.$row[5].'**'.$row[6].'" '.$selb.'  >'.$row[0].'-'.$row[1].' '.$row[3].'</option>';
			}	
		...
		<td colspan="2"><div align="right"><img src="imagenes/p_guardar1.gif" alt="Guardar Resultado" width="130" height="50" style="cursor:hand" onclick="Guardar('G');" 
			...
			<input name="guaocu" id="guaocu" type="hidden" value="" />
			</div></td>
	  </tr>
	</table>
