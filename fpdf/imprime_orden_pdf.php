...
if($obsi!=''){			              
    $pdf->MultiCell(19.5,0.5,'Observacion:'.$obsi,'TB','L',true);
			...
			if( $fi[1]!='' && $linea==1){
			   $linea=$pdf->GetY();
			   $pdf->Image($fi[1],16,$linea,4.5,4.5);}				  
		/*	if($inforesult[1]!='' ){
			$yprov=$pdf->GetY()+0.1;
            $pdf->SetDrawColor(100,100,100);//naranja (245,130,33)
            $pdf->SetFillColor(227,227,230);
            ...
            $pdf->SetFont('Arial','B',10);
            $pdf->SetXY(1,$yprov);
            $pdf->MultiCell(19.5,0.5,'OBSERVACIONES: ','TB','L',true);
			$yprov=$pdf->GetY();
            $pdf->SetDrawColor(255,255,255);//naranja (245,130,33)
 		    $pdf->SetFillColor(227,227,230);
			$pdf->SetTextColor(0,0,0);
			$pdf->SetFont('Arial','B',8);
			$pdf->Cell(4,0.5,$obsi,0,0,'L',false);
			}*/
////////////////////////fin/////////////////////////////////////////////////////////////////
if($con421!='0')
{
...
$pdf->SetFont('Arial','B',10);
$pdf->SetXY(1,$yprov);
$pdf->MultiCell(19.5,0.5,'AREA: SEROLOGIA','TB','L',true);
}$contt=1;
...
$pdf->SetFont('Arial','B',10);
$pdf->SetXY(1,$yprov);
$pdf->MultiCell(19.5,0.5,'AREA: COAGULACION','TB','L',true);
if($n>1) $pdf->Ln(0.1); else $pdf->Ln(0.3);
}$contt=1;
...
$linea=$pdf->GetY();
  ...
  $datos=explode('/*',$pac);
	...
	$pdf->Image('imagenes/umsl_claro.jpg',11,15,2,2);
	$pdf->SetXY(7,16);
