	...
	if($_POST["servi"]!=0)
	{
		...
		else
		{
			...
			while ($row2 = mysql_fetch_row($pac))
			{
				...
				$cont++;
				$nom=$row2[3].' '.$row2[4].' '.$row2[5].' '.$row2[6];
				echo '<tr class="textoN" '.$color.' >
						<td width="80" height="22">'.$row2[2].'</td>
						<td width="250"><a href="VisitaCompleta.php?visi='.$row2[0].'&idpac='.$row2[1].'&npac='.$nom.'&detvis='.$row2[9].'&servis='.$_POST["servi"].'">'.$nom.'</a></td>
						<td width="40">'.calculaedad($row2[11]).'</td>
						<td width="60">'.$row2[10].'</td>
						<td width="200">'.$row2[14].'</td>
