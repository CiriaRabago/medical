...
function buscar()
{
 ...
 {
	 document.form1.action='resu_exam.php';
	 document.form1.submit();
 }
 else
...
<?php   
     ...
     $cont=0;
     $ord= new orden('','','','','','','');
	 $resu=$ord->buscar_orden_pendiente();	 
	    ...
	    while ($row = mysql_fetch_row($resu))
		 {
		   ...
		   $usua=$bus1->consulta_usu();
	       $ce1=explode('**',$usua); 
		   if($ce1[8]==$condi || $condi=='1' || $row[5]<'2013-07-11 99:00:00')
			 {
		      if ($cont%2!=0) $color='bgcolor="#E3E3E6"'; else $color='';
		      $cont++;
              echo "<tr class='textoN' ".$color."><td><a href='resu_exam.php?orden=".$row[0]."'>".$row[0]."</a></td><td>".$row[1]."</td><td>".$row[5]."</td></tr>";
		      }
		   }	  
		 echo "</table>";
