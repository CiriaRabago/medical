...
function sincomillas(evt){
    ...
    function abrir() {
        ...
        check = document.getElementById("beneficiario");
        check2= document.getElementById("titular");
        if (check.checked) {
        	element.style.display='block';
        	check2.checked=false;
        	document.getElementById("cheq_titular").value=2; 
        }
        else {
            element.style.display='none';
            document.getElementById("cheq_titular").value='';
        }
    }
 function ocultar() {
        element = document.getElementById("contenido");
        check = document.getElementById("titular");
        check2 = document.getElementById("beneficiario");
        if (check.checked) {
        	check2.checked=false; 
            element.style.display='none';
            document.getElementById("cheq_titular").value=1;
        }else{
    ...
    function Requisitos() {//funcion basica para tildar la entrega de requisitos jose ramirez;;;;;;;;;;;;;;;;
        
        check_req= document.getElementById("req");
        if (check_req.checked) {
        	document.getElementById("cheq_req").value=2; 
        }else
        document.getElementById("cheq_req").value=1;
...
$_POST["ususis"]=$_SESSION["cedu_usu"];
 ...
 if(isset($_POST["ocu_g"]) && $_POST["ocu_g"]!='0' )
  { 
  	//condicion para guardar beneficiario
  	if($_POST["cheq_titular"]==1 ){//consulta o inserta beneficiario

  	$ben= new benef($_POST["ced"],$_POST["ced"],$_POST["nom1"].' '.$_POST["ape1"],$fecha_ac,$_SESSION["cedu_usu"],$_POST["te1"],$_POST["paren"]); 
    $regb=$ben->cons_benef3($_POST["ced"]);
	...
	if($_POST["cheq_titular"]==2 ){//consulta o inserta titular
      $ben= new benef($_POST["cedtit"],$_POST["ced"],$_POST["nomtit"],$fecha_ac,$_SESSION["cedu_usu"],$_POST["teltit"], $_POST["paren"]); 
       $regb=$ben->cons_benef3($_POST["ced"]);
       if($regb!=false)
	   { 
	...
	if($_POST["cheq_req"]==2 ){//insertar o modificar si un paciente entrego requisitos;;;;;;;;;;;;;;;; jose ramirez
		$requi= new pac_req('',$_POST["ced"],$_POST["req"],'A'); 
	  $reg_r=$requi->cons_pac_req2();
	  if($reg_r!='false'){
	   }else
	   $ins_req=$requi->ins_pac_req();
 
	 }
	if($_POST["cheq_req"]==1 ){//insertar o modificar si un paciente entrego requisitos;;;;;;;;;;;;;;;; jose ramirez
		$requi= new pac_req('',$_POST["ced"],'1','A'); 
	  $reg_r=$requi->cons_pac_req2();
	  if($reg_r!=false){
	  	$modi_r=$requi->eliminar_pac_req($_POST["ced"],'1');	
...
<img src="<?php echo $_POST["imafot"]?>?<?php=time()?>" width="120" height="100" id="foto" name="foto"><img src="imagenes/p_elifoto1.gif" alt="Eliminar la Foto" width="25" height="25" style="cursor:hand" onClick="eliminar_foto();"	onMouseOver="this.src='imagenes/a_elifoto1.gif'"  onMouseOut="this.src='imagenes/p_elifoto1.gif'">
    ...
    <tr>
	  ...
	  <span class="Estilo2"  >El paciente es el titular de la p&oacute;liza</span>
	   ...
	   ?>		
			...
			<tr>
			<td>
			<div class="etiqueta"> Estatus/Parentesco:</div></td>
			<td colspan="3">
			<select name="paren" class="texto" id="paren">
