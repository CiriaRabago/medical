<?php   
session_start();
if ($_SESSION["cedu_usu"]!='' && $_SESSION['cedu_usu']!='1001') {
...
list($year,$mes) = explode("-",$fecha_mes);
...
if ($_POST["ocu_b"]=='1')
  {
		...
		{	
			...
			//busco las visitas en el mes 
					$queryto="select nomb_servicio,fecha_ing_visita from slc_visita, slc_servicio where slc_paciente_id_paciente=$ced and fecha_ing_visita>'$year-$mes-00' and status_visita ='A' and ced_especialista <> 4 and ced_especialista <> 225928 and id_servicio=slc_servicio_id_servicio";
					$datos2=mysql_query($queryto);
			//termino la busqueda:::::::::::::::::::::::::::::::::::::::::::::::::::::::
...
?>
  <table width="683" border="0" align="center" <?php  echo $mosreg; ?>>
    <tr class="titulofor">
      <td colspan="3"><div align="center">Registro de llegada </div></td>
    </tr>
    <tr>
	...
	<tr>
	  <tr class="titulorep">
      <td colspan="2">Visitas este Mes</td>
      </tr>

      <?php //imprimo las visitas del mes:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
      while($fila=mysql_fetch_array($datos2))
			{?>
		<tr class="texto">
      <td colspan="2" class="texto"><?php echo $fila[0].' '.$fila[1];?></td>
    </tr>  
		<?php }//termino la impresion:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
		?>
	...
	<tr> <td class="etiqueta">Tipo de paciente :</td>
		<?php 
		$ben= new benef('','','',''); 
		$regb=$ben->cons_benef2($ced);
		if($regb)
		{ $benefe=explode('**',$regb);
		} if ($ced!='' && $ced==$benefe[0]) {?>
		...
		<?php 	
		}if ($ced!='' && $ced!=$benefe[0] && $benefe[0]!='') {?>
		
		<td class="etiqueta">Beneficiario del titular: <?php echo$benefe[2].' de CI: '.$benefe[0].', Tlf. '.$benfe[3];?></td>
		<?php 	
		}
	...
	<?php  }?>
	<tr> <td class="etiqueta">Requisitos :</td>
	<?php 
$requi= new pac_req('',$ced,'1','A'); 
	  $reg_r=$requi->cons_pac_req2();
	if ($reg_r!='' && $reg_r==1) { ?>
   
	<td class="etiqueta"> Consignados</td>

   <?php   }elseif($reg_r!='' && $reg_r!=1){
   	?>
   		<td class="mensaje"> Faltan Requisitos</td>
   	<?php 
   	}?> 
</td>
</tr>
