...
$pdf->AliasNbPages();
		...
		{	if ($_GET['medi']=='X') {
		   ...
		   $pdf->SetX(20);
		   $pdf->Cell(20,5,'N°',0,0,'L',false);
		   $pdf->Cell(70,5,'MEDICO',0,0,'L',false);
		   $pdf->Cell(80,5,'NOMBRE DEL SERVICIO',0,0,'L',false);
		   $pdf->Cell(40,5,'ESTADO',0,0,'L',false);
		   $pdf->Cell(20,5,'CANTIDAD',0,1,'L',false);
		   ...
		   { 
			...
			if ($_GET['medi']=='X') {
				...
				$ymax=$pdf->GetY();	$pdf->SetXY(40,$posy);
				$pdf->MultiCell(0,5,$row[4],0,'L',false);
				if($pdf->GetY()>$ymax) $ymax=$pdf->GetY();	$pdf->SetXY(110,$posy);
			   	$pdf->MultiCell(0,5,$row[1],0,'L',false);
				if($pdf->GetY()>$ymax) $ymax=$pdf->GetY();	$pdf->SetXY(190,$posy);
			   	$pdf->MultiCell(0,5,$sta,0,'L',false);
				if($pdf->GetY()>$ymax) $ymax=$pdf->GetY();	$pdf->SetXY(240,$posy);
		   ...
		   $pdf->SetFont('Arial','BI',10);
		   $pdf->SetX(40);
		   $pdf->Cell(200,5,'TOTAL',0,0,'L',false);
