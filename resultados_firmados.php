...
function buscar()
{
 ...
 {
	 document.form1.action='lista_resultado_firmado.php';
	 document.form1.submit();
 }
 else
...
<form name="form1" id="form1" method="post" action="resultados_firmados.php">
<table width="800" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr class="titulofor">
    <td colspan="4"><div align="center">RESULTADOS FIRMADOS</div></td>
  </tr>
  <tr>
  ...
  <tr>
    <td colspan="4" class="td-btn">
	  <div align="center">
	    <p class="textoN"><br>
	        Orden No. <input name="orden" id="orden" type="text" class="texto" onchange="buscar();" />
	        <br>
...
<?php   
     ...
     if((mysql_num_rows($resu))!=0)
	 {
	    ...
	    while ($row = mysql_fetch_row($resu))
		 {
		   $bus1= new usuario('','','','',$row[1],'','','');
		   $usua=$bus1->consulta_usu();
	       $ce1=explode('**',$usua); 
		   if($ce1[8]==$condi || $condi=='1')
			 {
			  $f=new firma($row[0],'','','','');
			  $fir=$f->consultar_firmados();
	          if($fir!=false)
			   {
		        if ($cont%2!=0) $color='bgcolor="#E3E3E6"'; else $color='';
		          $cont++;
                echo "<tr class='textoN' ".$color."><td><a href='lista_resultado_firmado.php?orden=".$row[0]."'>".$row[0]."</a></td><td>".$row[1]."</td><td>".$row[2]."</td></tr>";  } 
		      }
		   }	  
		 echo "</table>";
